...
export default function ProjetosPromocao() {
  ...
  // Get all active promotional projects
  const projetosPromocao = projetos?.filter(projeto => 
    projeto.promocao_ativa && 
    isPromotionActive(projeto.promocao_ativa, projeto.promocao_inicio, projeto.promocao_fim)
  ) ?? [];

  const projetosFiltrados = projetosPromocao.filter((projeto) => {
  ...
  return (
    <div className="flex-1 bg-gray-100 pt-24 pb-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {/* Header Section */}
        <div className="bg-gradient-to-r from-red-600 to-red-800 rounded-lg shadow-lg p-6 mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center space-x-4">
              <Building2 className="h-8 w-8 sm:h-10 sm:w-10 text-white" />
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-white">Projetos em Promoção</h1>
                <p className="mt-1 sm:mt-2 text-sm sm:text-base text-red-100">
            ...
            {projetosFiltrados.length === 0 ? (
              <div className="text-center py-12">
                <h3 className="mt-2 text-lg font-medium text-gray-900">
                  Nenhum projeto em promoção encontrado
                </h3>
